...
**Local KB check (articles/posts/raw text only):** Before fetching, call `mcp__local-kb__kb_search` with the topic or key terms. If `low_confidence` is false and results present, prepend them as context to Phase 3. Useful for topics in the corpus: German tax law, n8n, Anthropic/Claude docs, Arch Linux, Rust, TypeScript. Skip for GitHub repos (agents explore directly).
