---
description: |
  Autonomous cross-entity knowledge maintenance. Scans all entities for staleness, gaps, inconsistencies, and stalled roadmaps, then fixes what it can and flags the rest.
  Manual: when you want autonomous knowledge maintenance across all entities, or when multiple entities need attention.
---
...
- Entity dirs that exist in `$KB_DIR/{projects,areas}/` but aren't in MEMORY.md entity table -> flag as untracked
...
Read today's and yesterday's daily notes from `$KB_DIR/daily/`. Extract `decisions` and `outputs`. Check if any mention "TODO", "next step", "follow up", or "deferred". Cross-reference against entity backlogs and roadmaps - flag items that were deferred but not tracked.
