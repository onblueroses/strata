---
description: "Collect dmux worktree results and plan merge order. Triggers on: 'collect', 'collect results', 'gather the dispatched work', 'how are the tasks', 'are they done', 'check on the agents', 'what's the progress', 'dispatch status', 'dmux status', 'task status', 'are the panes done', 'merge the worktrees', 'pull in the work'. Also triggers after /dispatch when the user returns, mentions panes or worktrees, or ends with uncollected tasks. Pairs with /dispatch, /review, and /commit. Manual: /collect."
---
