...
name: commit
description: |
  Smart multi-commit. Groups uncommitted changes by logical intent and creates
  separate atomic commits. Runs /review once across everything, then commits
  each group in dependency order. No manual approval for grouping.
...
when_to_use: After review passes and changes are ready to commit
---
...
**Public repo message gate:** If this is a public repo (detected in /review step 2), validate the draft message before committing. The message MUST NOT contain any of:
- Process narration: "AI slop", "anonymize", "clean up public", "remove AI", "banned word", "review issues", "genericize", "tighten", "sanitize"
- Internal references: private project names (e.g. internal codenames from your CLAUDE.md), private IPs, internal tool names
- Cleanup meta-language: anything that reveals the commit is fixing a previous oversight ("remove leaked", "redact", "was accidentally")
- Verbose bodies: more than 2 lines (excluding Co-Authored-By). Subject-only is the default.

If the draft message fails this gate, rewrite it to describe WHAT changed technically, not WHY it was cleaned up. "Update README" not "Remove AI patterns from README". "Refactor path handling in skills" not "Genericize private paths".
