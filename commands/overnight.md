---
description: "Multi-hour unattended maintenance over the local knowledge base; builds a finite manifest of bounded work items upfront, then processes them sequentially with on-disk checkpointing. Compaction-safe and anti-drift via a single source-of-truth manifest JSON. Triggers on: 'overnight maintenance', 'run overnight', 'unattended cleanup', 'long maintenance pass', 'multi-hour audit', 'knowledge base sweep', 'staleness audit overnight', 'tidy everything while I sleep'. Also triggers when: user wants extended autonomous work across many entities/specs without per-item supervision; user wants a single resumable artifact that survives compaction; user wants structural tidy + spec graveyard + entity audit + enrichment chained as phases. Pairs with /spec (shared resumable-artifact pattern — spec persists implementation state, overnight persists maintenance state), /tidy (overnight Phase 2 wraps /tidy's checks; tidy alone for quick one-shot), /reconcile (downstream — overnight flags entities needing ground-truth verification), /loop (cousin — both run autonomously without intervention), /get-to-work (lenses 2/3/5/7 invoked in overnight Phase 4 sweep). Manual: invoke when running extended maintenance overnight or during long unattended periods."
---
...
```
/overnight              # Full run (up to 6 hours) - generates compaction cascade, avoid
/overnight --phase 1    # Phase 2 only: structural tidy + spec graveyard (~1h)
/overnight --phase 2    # Phase 3 only: entity staleness audit (~2h)
/overnight --phase 3    # Phase 4 only: enrichment + cross-refs + sweep (~1.5h)
/overnight --build      # Phase 1 only: build manifest, no fixes (replaces --dry-run)
/overnight --dry-run    # Alias for --build (deprecated)
```

**Preferred usage**: run phases separately on consecutive days rather than one 6-hour session. Each phase session stays under 2 hours, limiting context accumulation and compaction overhead.
...
- **No source code modification** - reads project code for enrichment, writes only to `$KB_DIR/`
- **No improvisation** - only processes items in the manifest. Discoveries become new manifest items.
- **No `.claude/` writes** - never create, edit, or delete files under `.claude/`. Follow-up files go in `$STATE_DIR/`.

## Phase Flag Behavior

Parse `$ARGUMENTS` for `--phase N`, `--build`, `--dry-run` before doing anything else.

- **`--phase 1`**: requires manifest exists (status `running` or `complete`). Execute Phase 2 items only. Deadline = now + 2h.
- **`--phase 2`**: requires manifest exists. Execute Phase 3 items only. Deadline = now + 2h.
- **`--phase 3`**: requires manifest exists. Execute Phase 4 items only. Deadline = now + 2h.
- **`--build` / `--dry-run`**: run Phase 1 (manifest construction) only, then stop. No deadline needed.
- **No flag**: full run, deadline = now + 6h. Warn: "Full run can generate significant compaction overhead. Consider using `--build` first, then `--phase 1/2/3` on separate sessions."

If `--phase N` is given and no manifest exists: stop with "No manifest found. Run `/overnight --build` first to build the work queue."

If `--phase N` is given and the targeted phase's items are all `done` or `skipped`: report "Phase N already complete" and stop.

## Resume Logic

Check for existing manifest at `$STATE_DIR/overnight-manifest.json`:

- **Status `running`**: resume from first `pending` item in the target phase (or globally if no `--phase` flag). Do NOT rebuild the manifest.
- **Status `complete`**: warn "Previous overnight run completed. Start fresh?" and stop.
- **No manifest**: if `--phase N`, stop with error (see above). Otherwise proceed to Phase 1 (build manifest).

After compaction: read the manifest. Find the first `pending` item in the current phase scope. Continue from there.
...
3. Create manifest JSON at `$STATE_DIR/overnight-manifest.json` with skeleton:
...
4. Create follow-up file at `$STATE_DIR/followup-{session-id}.md`.
...
Read first 5 lines of all spec files in `$SPECS_DIR/`. For each with `Status: in-progress` and `Last updated` older than 7 days, append:
...
From the entity list, identify entities matching ANY of: (a) summary.md under 500 chars (excluding frontmatter), (b) items.json missing, (c) both. Each qualifying entity gets one enrichment item listing all reasons:
...
Iterate `category == "spec-triage"` items. For each stale in-progress spec (7+ days since last update):
...
- If items.json is missing: read the entity's summary.md and project source to identify structured details (gotchas, config values, ports, deploy commands, dependency versions) that belong in items.json. Create with real entries - not empty stubs. If nothing structured is worth capturing, create `[]`.
- Respect source-of-truth rules: summary.md owns narrative, items.json owns structured details. No duplication.
...
Commits: [hashes]
Follow-up: $STATE_DIR/followup-{session-id}.md (N items)
...
- Delete files - use `~/to-delete/` convention
- Push to any remote
...
- Write, edit, or delete anything under `.claude/` - requires explicit user permission
