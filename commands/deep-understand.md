---
description: |
  Deep familiarization with any codebase or project. Adaptive agents based on project size.
  Auto-trigger: when exploring an unfamiliar codebase or directory for the first time.
---
...
- **Skip if** no argument provided AND cwd is the home directory (`$HOME`) - too broad; ask the user for a specific target
...
3. **Classify project size** using the **Project Size** system in `$STRATA_HOME/reference/tier-classification.md`:
   - **Small:** < 30 files
   - **Medium:** 30-200 files
   - **Large:** 200+ files
...
**Observability:** After each batch completes, append one log entry per agent to `$STATE_DIR/agent-log.jsonl`:
...
Agent count and model by size (see `$STRATA_HOME/reference/tier-classification.md` for full details):
...
2. **Scratchpad written** - is `.claude/scratchpad-deep-understand.md` actually saved to disk?
