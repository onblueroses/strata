---
description: |
  Adaptive research - scales from direct lookup to parallel deep-dive based on question complexity.
  Auto-trigger: when user asks a research question, "look into X", or needs information beyond the codebase.
---
...
Other agents are covering: [comma-separated list of the other sub-questions]
Focus only on your assigned question - do not duplicate their territory.
...
After each agent completes (success or failure), append a log entry to `$STATE_DIR/agent-log.jsonl`:
```
...
| Source | "Various online sources" | "[Flask-Caching docs](URL) - confirms Redis is the recommended backend for production" |
