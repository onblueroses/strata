...
description: "Actively build and sharpen a project's domain model as you design: map multiple bounded contexts, invent edge-case scenarios that force precise concept boundaries, cross-reference stated behavior against the code and surface contradictions, and write the glossary down inline as terms crystallize. This is the write-side counterpart to /decision-mapping; when you are actively building the domain model or writing an ADR, /decision-mapping hands off here. Use when the model is changing, not merely being consumed: pinning down a ubiquitous language, splitting or naming bounded contexts, or recording an architectural decision. Triggers on: 'domain model', 'ubiquitous language', 'bounded context', 'context map', 'glossary', 'name this concept', 'model the domain', 'is this term right', 'write an ADR'. Also triggers when: a design conversation keeps reusing a fuzzy term, a new subsystem needs a context boundary drawn, or stated behavior and code seem to disagree about what a concept means. Pairs with /decision-mapping (upstream decision-resolution interview that hands off here once the model starts changing), /spec (downstream once the model is locked), /recon (upstream verified brief feeds the model). Manual: /domain-modeling [topic]."
allowed-tools:
  - Read
  - Grep
  - Glob
