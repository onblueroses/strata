#!/usr/bin/env bash
# context-doc-router.sh - fires on UserPromptSubmit
# Matches incoming prompt against keyword lists embedded in reference docs.
# Injects Quick Nav sections of matched docs as system reminders.
# Fires once per doc per session (session flag file deduplication).

REFS_DIR="$STRATA_HOME/reference"
MANDATORY_DOCS=()
...
for docFile in "$REFS_DIR"/*.md; do
    ...
    for kw in "${kw_array[@]}"; do
        ...
        # Skip keywords shorter than 6 chars to avoid false positives (e.g. "token", "hook", "node")
