#!/usr/bin/env bash
# PostToolUse hook: appends structured JSONL events for session state reconstruction.
# Edit/Write -> "edit" event with file path
# Bash git commit -> "commit" event with message
# Used by context-resume to replay session state after compaction.
...
jsonlFile="$STATE_DIR/session-events-$sessionId.jsonl"
